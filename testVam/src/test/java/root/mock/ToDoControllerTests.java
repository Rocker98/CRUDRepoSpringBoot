...
public class ToDoControllerTests {
    ...
    public void findAll(){
        ...
        try {
            mockMvc.perform(MockMvcRequestBuilders.get("/todos")).andExpect(MockMvcResultMatchers.status().isOk());
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//        }
    ...
    public void createTodo(){
        ...
        String lastName = faker.name().lastName();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("{\"todo\":\"Test task\",\"user\":{\"id\":1,\"user_name\":\"" + firstName + "\",\"surname\":\"" + lastName + "\"}}");
        try {
            mockMvc.perform(MockMvcRequestBuilders.post("/users/1/todo").contentType(MediaType.APPLICATION_JSON)
